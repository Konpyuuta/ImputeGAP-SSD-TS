...
import pandas as pd
from imputegap.wrapper.AlgoPython.MOMENT.momentfm import MOMENTPipeline
from imputegap.wrapper.AlgoPython.MOMENT.momentfm.utils.utils import control_randomness
...
def load_moment_imputer_model(model_name="AutonLab/MOMENT-1-large"):
    # First we need to load the pre-trained model for imputation ..
    model = MOMENTPipeline.from_pretrained(
        model_name,
...
def impute_time_series(series, moment_model=None):
    if not isinstance(series, pd.Series):
        series = pd.Series(series)

    x = series.values.reshape(1, -1)
    output = moment_model(x)
    reconstructed = output["reconstruction"]
    imputed = reconstructed.reshape(-1)
    return pd.Series(imputed, index=series.index)