# ===============================================================================================================
# SOURCE: https://github.com/decisionintelligence/SSD-TS/
#
# THIS CODE HAS BEEN MODIFIED TO ALIGN WITH THE REQUIREMENTS OF IMPUTEGAP (https://arxiv.org/abs/2503.15250),
#   WHILE STRIVING TO REMAIN AS FAITHFUL AS POSSIBLE TO THE ORIGINAL IMPLEMENTATION.
#
# FOR ADDITIONAL DETAILS, PLEASE REFER TO THE ORIGINAL PAPER:
# https://arxiv.org/abs/2410.13338
# ===============================================================================================================
...
from utils.util import get_mask_mnr, get_mask_bm, get_mask_rm, find_best_model
...
def impute(output_directory,
             ...
             masking,
             missing_k,
             only_generate_missing, incomp_data):
    """
    Generate data based on ground truth
    ...
    ckpt_path = os.path.join(ckpt_path, local_path)
    if ckpt_iter == 'max':
        ckpt_iter = find_best_model(ckpt_path)
    model_path = os.path.join(ckpt_path, '{}.pkl'.format(ckpt_iter))
    try:
        checkpoint = torch.load(model_path, map_location='cpu')
        net.load_state_dict(checkpoint['model_state_dict'], strict=False)
    ...
    generated_samples = []
    if masking == 'mnr':
        mask_T = get_mask_mnr(incomp_data, missing_k)
        mask = mask_T.permute(1, 0)
        mask = mask.repeat(incomp_data.size()[0], 1, 1)
        mask = mask.type(torch.float).cuda()

    elif masking == 'bm':
        mask_T = get_mask_bm(incomp_data[0], missing_k)
        mask = mask_T.permute(1, 0)
        mask = mask.repeat(incomp_data.size()[0], 1, 1)
        mask = mask.type(torch.float).cuda()

    elif masking == 'rm':
        mask_T = get_mask_rm(incomp_data[0], missing_k)
        mask = mask_T.permute(1, 0)
        mask = mask.repeat(incomp_data.size()[0], 1, 1).float().cuda()

    batch = incomp_data.permute(0, 2, 1)
